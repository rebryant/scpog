...
MethodManager *MethodManager::makeMethodManager(po::variables_map &vm,
  ...
  out << "c [INITIAL INPUT] Statistics about the input "
         "formula\n";
...
void MethodManager::displayInfoVariables(ProblemManager *problem,
  ...
  if (selected.size()) {
    out << "c [SELECTED VARIABLES] list: ";
    std::sort(selected.begin(), selected.end());
    for (auto v : selected) out << v << " ";
    out << "\n";
  }

  std::vector<Var> &forget = problem->getForgetVar();
  if (forget.size()) {
    int count = 0;
    out << "c [FORGET VARIABLES] list: ";
    std::sort(forget.begin(), forget.end());
    for (auto v : forget) {
	if (++count > 100) {
	    out << "...";
	    break;
...
ProblemManager *MethodManager::runPreproc(po::variables_map &vm,
  ...
  // REB: Remove screen control characters
  out << "c [MAIN PREPROCESSED INPUT] Statistics about the "
