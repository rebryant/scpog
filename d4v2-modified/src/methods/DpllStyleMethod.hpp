...
// REB: Enable use of cutsets for projected KC
#define CUT 1
...
class DpllStyleMethod : public MethodManager, public Counter<T> {
 private:
  ...
  bool m_skolemize;
  // REB: Control amount of information printed
  bool m_quiet;
  ...
  std::vector<bool> m_isForgetVariable;
  std::vector<bool> m_currentPrioritySet;
  // How much decreased weight should forget variables be given in variable selection
  double m_weakWeight = 1.0;
 ...
 public:
  ...
  DpllStyleMethod(po::variables_map &vm, std::string &meth, bool isFloat,
    ...
    // These include variables declared as "show" and "forget"
    ...
    m_isProjectedMode = m_problem->getNbSelectedVar() > 0 && m_problem->getNbSelectedVar() < m_problem->getNbVar();

    // specify which variables are forget vars
    m_isForgetMode = false;
    m_isForgetVariable.clear();
    m_isForgetVariable.resize(m_problem->getNbVar() + 1, false);
    for (auto v : m_problem->getForgetVar()) m_isForgetMode = m_isForgetVariable[v] = true;

    m_currentPrioritySet.resize(m_problem->getNbVar() + 1, false);
    const char *mode_string;
    if (m_isProjectedMode) 
      mode_string = m_isForgetMode ? "Project+Forget" : "Project";
    else
      mode_string = m_isForgetMode ? "Forget" : "Classic";
    m_out << "c [MODE] " << mode_string << '\n';
    // select the partitioner regarding if it projected model counting or not.
    if (!CUT && m_isProjectedMode)
    ...
    m_skolemize = vm["skolemize"].as<bool>();
    if (m_skolemize) {
      m_solver->setNeedModel(true);
      m_out << "c [MODE] Skolemization enabled" << '\n';
    }
    // REB: Set weight for variable selection
    m_weakWeight = vm["weak-weight"].as<double>();
    m_out << "c [MODE] Weak variable weight = " << m_weakWeight << '\n';

    m_quiet = vm["quiet"].as<bool>();
    m_callPartitioner = 0;
    m_usePartitioner = 0;
 ...
 private:
  /**
     ...
     Used for partitioning, and so retain forget variables
  ...
  /**
     Expel from a set of variables the ones they are marked as being decidable.
     Used for trimming formula, and so expel literals of forget variables
  ...
  void expelNoDecisionLit(std::vector<Lit> &lits,
                          std::vector<bool> &isDecisionVariable,
			  std::vector<bool> &isForgetVariable) {
    if (!m_isProjectedMode && !m_isForgetMode) return;
    ...
    for (unsigned i = 0; i < lits.size(); i++) {
      Var v = lits[i].var();
      if (isDecisionVariable[v] && !isForgetVariable[v]) lits[j++] = lits[i];
    }
    lits.resize(j);
  }  // expelNoDecisionLit
  ...
  */
  ...
  inline void printFinalStats(std::ostream &out) {
    // REB: Remove screen control characters
    separator(out);
    out << "c\n";
    out << "c Statistics \n";
    out << "c Compilation Information\n";
    ...
    out << "c Number of partitioner uses/calls: " << m_usePartitioner << "/" << m_callPartitioner << "\n";
    out << "c\n";
  ...
  */
  U compute_(std::vector<Var> &setOfVar, std::vector<Lit> &unitsLit,
    ...
    if (nbComponent) {
      ...
      // Exclude literals of bound variables from NNF
      expelNoDecisionLit(unitsLit, m_isDecisionVariable, m_isForgetVariable);
  ...
  */
  U computeDecisionNode(std::vector<Var> &connected, std::ostream &out) {
    ...
    // Looking for properties of current cutset
    bool hasFreeVariable = false;     
    bool hasPriorityVariable = false;

    if (m_isForgetMode || m_isProjectedMode) {
      for (auto v : connected) {
	if (m_specs->varIsAssigned(v))
	  continue;

	if (m_isDecisionVariable[v] && !m_isForgetVariable[v])
	  hasFreeVariable = true;

	if (m_currentPrioritySet[v])
	  hasPriorityVariable = true;

	if (hasFreeVariable && hasPriorityVariable)
	  break;
      }
    } else {
      for (auto v : connected) {
    ...
    if (hasFreeVariable) {
      if (!hasPriorityVariable && m_hCutSet->isReady(connected)) {
    ...
    // search the next variable to branch on.  Give preference to decision variables
    Var v = hasFreeVariable ?
      m_hVar->selectVariableWeighted(connected, *m_specs, m_currentPrioritySet, 
				     m_isForgetVariable, m_weakWeight) : 
      var_Undef;

    if (v == var_Undef) {
      unsetCurrentPriority(cutSet);
      // REB: Alternate handling of tautology
      return m_skolemize ? m_operation->manageSolution(connected) : m_operation->manageTop(connected);
    }
