...
int ParserDimacs::parse_DIMACS_main(BufferRead &in,
  ...
  for (;;) {
    ...
    if (in.currentChar() == 'p') {
      ...
      if (in.currentChar() != 'p') {
	    ...
	    readListIntTerminatedByZero(in, problemManager->getSelectedVar());
	} else if (in.canConsume("forget")) {
            readListIntTerminatedByZero(in, problemManager->getForgetVar());
	}
        else
          in.skipLine();
      }
  ...
  // Selected Vars = Show vars + Forget Vars
  std::vector<Var> &selected = problemManager->getSelectedVar();
  std::vector<Var> &forget = problemManager->getForgetVar();  
  for (auto v : forget)
