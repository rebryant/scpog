...
SpecManagerCnf::SpecManagerCnf(ProblemManager &p) : m_nbVar(p.getNbVar()) {
  ...
  // Only select variables can be cut
  if (p.getNbSelectedVar() > 0) {
      m_var_nocut.resize(m_nbVar + 1, true);
      m_nocut_count = m_nbVar;
      for (auto v : p.getSelectedVar()) { m_var_nocut[v] = false; m_nocut_count--; }
  } else {
      m_var_nocut.resize(m_nbVar + 1, false);
      m_nocut_count = 0;
  }
