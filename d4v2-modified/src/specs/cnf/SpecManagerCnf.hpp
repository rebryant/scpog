...
class SpecManagerCnf : public SpecManager {
 protected:
  ...
  // REB: Vector indicating which variables can't be cut
  std::vector<bool> m_var_nocut;
  int m_nocut_count;
 ...
 public:
  ...
  inline int varNocutCount() override {
    return m_nocut_count;
  }

  // About the variable
  inline bool varIsNocut(Var v) override {
    return m_var_nocut[v];
  }
