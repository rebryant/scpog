...
Var ScoringMethod::selectVariableWeighted(std::vector<Var> &vars, SpecManager &s,
					  std::vector<bool> &isDecisionVariable,
					  std::vector<bool> &isWeakVariable, 
					  double weakWeight) {
  Var ret = var_Undef;
  double bestScore = -1;
  // Debug
  ...
  // Debug
  //  const char *dstring = isWeak ? " (weak) " : " (strong) ";
  //  std::cout << "c [VAR] Selected variable " << ret << dstring << "score = " << bestScore << 
  //      ". Best of " << count << " variables" << '\n';

  return ret;
}  // selectVariableWeighted
